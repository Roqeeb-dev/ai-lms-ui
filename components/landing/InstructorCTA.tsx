...
export default function InstructorCTA() {
  ...
  return (
    <section
      id="instructors"
      className="py-24 px-4 md:px-8 bg-secondary relative overflow-hidden"
    >
      <div
        className="pointer-events-none absolute inset-0 opacity-5"
        style={{
          backgroundImage:
            "radial-gradient(circle, var(--secondary-foreground) 1px, transparent 1px)",
          backgroundSize: "24px 24px",
        }}
      />
      <div className="pointer-events-none absolute -top-20 -right-20 w-80 h-80 rounded-full bg-primary opacity-10 blur-3xl" />
      <div className="pointer-events-none absolute -bottom-20 -left-20 w-80 h-80 rounded-full bg-accent opacity-10 blur-3xl" />
      ...
      <div className="relative max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <div className="flex flex-col gap-3">
          {perks.map((perk) => (
            <div
              key={perk}
              className="flex items-center gap-3 rounded-xl border border-secondary-foreground/10 bg-secondary-foreground/5 px-4 py-3.5"
            >
